...
(function(mod) {
    ...
    function skipBlockComment() {
        ...
        tokPos = end + 2;
        // @if LOCATIONS=true
        var comment = input.slice(start + 2, end);
    ...
    function parse_SwitchStatement() {
        var empty = true, sawDefault = false;
        next();
        parseParenExpression();
        expect(_braceL);
    ...
    function parse_TryStatement() {
        ...
        if (tokType === _catch) {
            next();
            expect(_parenL);
            var param = parse_Identifier();
            if (strict && isStrictBadIdWord(param.name))
                raise(tokPos, 'Binding ' + param.name + ' in strict mode');
            expect(_parenR);
            parse_BlockStatement();
            handler = 1;
        }
    ...
    function parse_BlockStatement() {
        var strict = false, oldStrict;
        expect(_braceL);
        if (eat(_braceR) === false) {
            for(;;) {
                if (isUseStrict(parseStatement())) {
                    oldStrict = strict;
    ...
    function parseMaybeAssign(noIn) {
        if (tokType.isAssign) {
            checkLVal(parseMaybeConditional(noIn));
            next();
            parseMaybeAssign(noIn);
        } else {
    ...
    function parseExprOp(minTokType, noIn) { // what to do about this garbage producing noIn?
        var curTokType = tokType;

        if (curTokType.precedence !== 0 && curTokType.precedence > minTokType.precedence && (!noIn || tokType !== _in)) {

            next();
            return parseExprOp(minTokType, noIn);
        }
    }
    ...
    function parseExprAtom() {
        switch (tokType) {
            ...
            case _bracketL: next(); return parse_ArrayExpressionList();
            case _function: next(); return parseFunction();
