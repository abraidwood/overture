...
//        tokCommentsBefore = tokComments;
 ...
 //     if (options.locations) tokStartLoc = new line_loc_t();
    ...
    function parse_BreakStatement() {
        var starttype = tokType, label = null;
        next();
        ...
        check_label_exists(label, true, starttype);
    }
    ...
    function parse_ContinueStatement() {
        ...
        check_label_exists(label, false, starttype);
    }
    ...
    function parseStatement() {
        switch (tokType) {
            case _break: parse_BreakStatement(); break;
            case _continue: parse_ContinueStatement(); break;
            case _debugger: parse_DebuggerStatement(); break;
            case _do: parse_DoWhileStatement(); break;
            case _for: parseStatement_for(); break;
            case _function: parse_Function(); break;
            case _if: parse_IfStatement(); break;
            case _return: parse_ReturnStatement(); break;
            case _switch: parse_SwitchStatement(); break;
            case _throw: parse_ThrowStatement(); break;
            case _try: parse_TryStatement(); break;
            case _var: parseStatement_var(); break;
            case _while: parse_WhileStatement(); break;
            case _with: parse_WithStatement(); break;
            case _braceL: parse_BlockStatement(); break;
            case _semi: parse_EmptyStatement(); break;

            case _slash:
                readToken_forceRegexp();

            default:
                parseStatement_default();
        }
    }
    ...
    function parse_ForStatement() {
        expect(_semi);
        if (tokType !== _semi) parseExpression(false);
        expect(_semi);
        if (tokType !== _parenR) parseExpression(false);
        expect(_parenR);
        parseStatement();
    }
    ...
    function parseExprOp(left, minTokType, noIn) { // what to do about this garbage producing noIn?
        ...
        if (curTokType.binop !== 0 && curTokType.binop > minTokType.binop && (!noIn || tokType !== _in)) {
            ...
            next();
            parseExprOp(parseMaybeUnary(), curTokType, noIn);
    ...
    function parse_NewExpression() {
        next();
        parseSubscripts_nocalls(parseExprAtom(false));
        if (eat(_parenL) === true) parse_ExpressionList();
    }
    ...
    function parseFunction() {
        if (tokType === _name) parse_Identifier();
    ...
    function parse_ArrayExpressionList() {
